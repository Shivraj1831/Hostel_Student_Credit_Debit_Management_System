...
//###################################
$sqlst = "SELECT student.*,course.course,(SELECT ifnull(SUM(transaction_amt),0) FROM `transaction` WHERE transaction_type='Credit' AND transaction_status='Active' AND student_id='$rstransaction[student_id]' AND transaction.transaction_id<'$_GET[transaction_id]') as creditamt,(SELECT ifnull(SUM(transaction_amt),0) FROM `transaction` WHERE transaction_type='Debit' AND transaction_status='Active' AND student_id='$rstransaction[student_id]' AND transaction.transaction_id<'$_GET[transaction_id]') as debitamt FROM student LEFT JOIN course ON student.course_id=course.course_id WHERE student.student_id = '" . $rstransaction['student_id'] . "' ";
