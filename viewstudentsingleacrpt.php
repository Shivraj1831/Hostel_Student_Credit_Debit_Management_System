          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">View Account Report of - <?php echo $rsstudentprofile['student_name']; ?> (<?php echo $rsstudentprofile['admission_no']; ?>)</h3>
              </div>
              <!-- /.card-header -->
...
<table class="table table-bordered table-striped">
				...
				<table class="table table-bordered table-striped">
				  ...
				  <tbody>
				  <?php
				  $sqltransaction = "SELECT student.*,transaction.*,staff.staff_name FROM transaction LEFT JOIN student ON student.student_id=transaction.student_id LEFT JOIN staff ON staff.staff_id=transaction.staff_id WHERE transaction.student_id='" . $_GET['student_id'] . "' AND transaction.transaction_status='Active' ORDER BY transaction.transaction_id,transaction.receipt_no";
				  ...
				  while($rstransaction = mysqli_fetch_array($qsqltransaction))
				  {
					echo "<tr>
						...
						<td>" . $rstransaction['transaction_type'] . "</td>
						<td>" . date("d-m-Y",strtotime($rstransaction['transaction_date'])) . "</td>
						<td>" . $rstransaction['transaction_note'] . "</td>";
					...
					echo "<td style='text-align: right;'>$currency " . $crdr . "</td>";
					echo "<td><center>";
					if($rstransaction['transaction_type'] == "Credit")
					{
					echo "<a href='crinvoice.php?transaction_id=$rstransaction[transaction_id]' target='_blank' class='btn btn-info' ><i class='fas fa-table'></i><br>View Report</a>";
					}
					if($rstransaction['transaction_type'] == "Debit")
					{
					echo "<a href='drinvoice.php?transaction_id=$rstransaction[transaction_id]' target='_blank' class='btn btn-info' ><i class='fas fa-table'></i><br>View Report</a>";
					}
					echo "</center><br>";
					echo "</td></tr>";
				  }
